<script lang="ts">
  ...
  import { makeOpenAIRequest } from '$lib/analyzer';
  import type { 
    FirestoreRepo, 
    AnalysisResult,
    Subsystem,
  ...
  // Render Mermaid diagram after update
  afterUpdate(async () => {
  ...
  function generateFrameworkRelationships(subsystems: Subsystem[], framework: Framework): string {
    let relationships = '';
    
    const getNodeIndex = (name: string) => subsystems.findIndex(s => s.name === name);
  ...
  function isEntryPointSubsystem(subsystem: Subsystem, framework: Framework): boolean {
    const name = subsystem.name.toLowerCase();
    return name.includes('routes') || name.includes('pages') || name.includes('endpoints');
  }
  ...
  function isServiceSubsystem(subsystem: Subsystem, framework: Framework): boolean {
    const name = subsystem.name.toLowerCase();
    return name.includes('services') || name.includes('api') || name.includes('stores');
  }
  ...
  async function renderMermaidDiagram() {
    ...
    try {
      ...
      // Render diagram
      const { svg } = await mermaid.default.render('architecture-diagram', mermaidDiagram);
      ...
      diagramContainer.innerHTML = '<p class="error">Failed to render diagram</p>';
    }
  }
    ...
    <div class="graph-container">
      ...
      <ion-card class="controls-card">
        <ion-card-header>
          ...
          <ion-card-subtitle>{repo.fullName} - {analysis.framework} ({analysis.subsystems.length} subsystems)</ion-card-subtitle>
        </ion-card-header>
        ...
        <ion-card-content>
          ...
          <div class="control-buttons">
            ...
            <ion-button size="small" fill="outline" onclick={() => showLegend = !showLegend}>
              <ion-icon name={showLegend ? 'eye-off' : 'eye'} slot="start"></ion-icon>
              {showLegend ? 'Hide' : 'Show'} Legend
            </ion-button>
            <ion-button size="small" fill="outline" onclick={() => switchDiagramType(diagramType)}>
              <ion-icon name="refresh" slot="start"></ion-icon>
              Refresh
            </ion-button>
          </div>
